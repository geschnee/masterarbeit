...
\begin{figure}[!h]
    ...
    \begin{tikzpicture}[%
        ...
        \draw[decorate,decoration={brace,mirror,amplitude=5pt}] (0+ \xOffset,-6+ \yOffset) -- (7+ \xOffset,-6+ \yOffset)
        ...
        \draw[decorate,decoration={brace,mirror,amplitude=5pt}] (8+ \xOffset,-6+ \yOffset) -- (11+ \xOffset,-6+ \yOffset)
        ...
        \node[right] at (\xOffset + 11 + 1, -2.5 + \yOffsetHalf) {action};

        \draw[->,thick, red] (0, -2.5)--(-0.5,-2.5)--(-0.5,-2.5+\yOffset)--(\xOffset, -2.5+ \yOffset);
        \node[right] (camArrow) at (-2, -2.5 + \yOffsetHalf) {\agentImg{1}};
        
        \node[below] at (camArrow.south) {rewards + new agent camera image};

    \end{tikzpicture}
    \caption{Full agent interaction with the environment, communication in red}
    \label{fig:agent_interaction}
\end{figure} % this replaces the old rl_cycle image

%\node[inner sep=0pt] (step3) at (\bufferXOffsets + 0.3,\imagesOffsetY + 0.3)
%{\preprocessedImg{3}};
%\node[inner sep=0pt] (step2) at (\bufferXOffsets + 0.2,\imagesOffsetY + 0.2)
%{\preprocessedImg{2}};
%\node[inner sep=0pt] (step1) at (\bufferXOffsets + 0.1,\imagesOffsetY + 0.1)
%{\preprocessedImg{1}};
%\node[inner sep=0pt] (step0) at (\bufferXOffsets,\imagesOffsetY)
%{\preprocessedImg{0}};
