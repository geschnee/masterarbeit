...
The data collection algorithm resets the environment according to the parameters trainingMapType, trainingLightSetting and spawn\_point \ref{training_settings}. The observation is used to prompt the model for an action. The action is then applied to the environment, resulting in a reward and a new environment state. The observation, action and reward tuples are stored in the replay buffer. A new episode is started upon termination of the current episode. The data collection process continues until the replay buffer is full.
...
% figure training settings trainingMapType and trainingLightSetting
...
\begin{figure}
    ...
    \begin{tabular}{|| c | p{0.25\linewidth} | p{0.4\linewidth} ||} 
        ...
        \hline
        \multirow{4}{*}{spawn\_point} & Fixed & Spawn JetBot with fixed coordinates and orientation. \\\cline{2-3}
        & OrientationRandom & Spawn JetBot with fixed coordinates and random orientation (-15 to 15 degrees). \\\cline{2-3}
        & OrientationVeryRandom & Spawn JetBot with fixed coordinates and random orientation (-45 to 45 degrees). \\\cline{2-3}
        & FullyRandom & Spawn JetBot with random coordinates and random orientation (-45 to 45 degrees) \\
        \hline
    \end{tabular}
    \end{center}
